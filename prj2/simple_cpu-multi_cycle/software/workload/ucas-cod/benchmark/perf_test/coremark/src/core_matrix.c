...
ee_u32 core_init_matrix(ee_u32 blksize, void *memblk, ee_s32 seed, mat_params *p) {
	...
	p->A=A;
	p->B=B;
	p->C=(MATRES *)align_mem(B+mul(N,N));
	p->N=N;
#if CORE_DEBUG
	printmat(A,N,"A");
	printmat(B,N,"B");
...
void matrix_mul_const(ee_u32 N, MATRES *C, MATDAT *A, MATDAT val) {
	...
	for (i=0; i<N; i++) {
		for (j=0; j<N; j++) {
			C[mul(i,N)+j]=mul((MATRES)A[mul(i,N)+j] , (MATRES)val);
		}
	}
}
...
void matrix_mul_vect(ee_u32 N, MATRES *C, MATDAT *A, MATDAT *B) {
	...
	for (i=0; i<N; i++) {
		C[i]=0;
		for (j=0; j<N; j++) {
			C[i]+=mul((MATRES)A[mul(i,N)+j] , (MATRES)B[j]);
		}
	}
}
...
void matrix_mul_matrix(ee_u32 N, MATRES *C, MATDAT *A, MATDAT *B) {
	...
	for (i=0; i<N; i++) {
		for (j=0; j<N; j++) {
			...
			for(k=0;k<N;k++)
			{
				C[mul(i,N)+j]+=mul((MATRES)A[mul(i,N)+k] , (MATRES)B[mul(k,N)+j]);
			}
		}
	}
}
...
void matrix_mul_matrix_bitextract(ee_u32 N, MATRES *C, MATDAT *A, MATDAT *B) {
	...
	for (i=0; i<N; i++) {
		for (j=0; j<N; j++) {
			...
			for(k=0;k<N;k++)
			{
				MATRES tmp=mul((MATRES)A[mul(i,N)+k] , (MATRES)B[mul(k,N)+j]);
				C[mul(i,N)+j]+=mul(bit_extract(tmp,2,4),bit_extract(tmp,5,7));
			}
		}
	}
}
