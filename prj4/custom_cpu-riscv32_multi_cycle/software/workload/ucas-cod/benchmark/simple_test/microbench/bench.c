...
int main() {
  ...
  for (int i = 0; i < ARR_SIZE(benchmarks); i ++) {
    ...
    if (msg != NULL) {
      ...
      unsigned long mr_cnt = ULONG_MAX;
      unsigned long mw_cnt = ULONG_MAX;
      unsigned long inst_req_delay_cnt = ULONG_MAX;
      unsigned long inst_delay_cnt = ULONG_MAX;
      unsigned long mr_req_delay_cnt = ULONG_MAX;
      unsigned long rdw_delay_cnt = ULONG_MAX;
      unsigned long mw_req_delay_cnt = ULONG_MAX;
      unsigned long branch_inst_cnt = ULONG_MAX;
      unsigned long jump_inst_cnt = ULONG_MAX;
      ...
      for (int i = 0; i < REPEAT; i ++) {
        ...
        if (res.mr_cnt < mr_cnt) mr_cnt = res.mr_cnt;
        if (res.mw_cnt < mw_cnt) mw_cnt = res.mw_cnt;
        if (res.inst_req_delay_cnt < inst_req_delay_cnt) inst_req_delay_cnt = res.inst_req_delay_cnt;
        if (res.inst_delay_cnt < inst_delay_cnt) inst_delay_cnt = res.inst_delay_cnt;
        if (res.mr_req_delay_cnt < mr_req_delay_cnt) mr_req_delay_cnt = res.mr_req_delay_cnt;
        if (res.rdw_delay_cnt < rdw_delay_cnt) rdw_delay_cnt = res.rdw_delay_cnt;
        if (res.mw_req_delay_cnt < mw_req_delay_cnt) mw_req_delay_cnt = res.mw_req_delay_cnt;
        if (res.branch_inst_cnt < branch_inst_cnt) branch_inst_cnt = res.branch_inst_cnt;
        if (res.jump_inst_cnt < jump_inst_cnt) jump_inst_cnt = res.jump_inst_cnt;
      }
      ...
      printk("\tInstruction Count: at least %u\n", inst_cnt);
      printk("\tMemory Read Instruction Count: at least %u\n", mr_cnt);
      printk("\tMemory Write Instruction Count: at least %u\n", mw_cnt);
      printk("\tInstruction Fetch Request Delay Cycle: at least %u\n", inst_req_delay_cnt);
      printk("\tInstruction Fetch Delay Cycles: at least %u\n", inst_delay_cnt);
      printk("\tMemory Read Request Delay Cycles: at least %u\n", mr_req_delay_cnt);
      printk("\tRead Data Delay Cycles: at least %u\n", rdw_delay_cnt);
      printk("\tMemory Write Request Delay Cycles: at least %u\n", mw_req_delay_cnt);
      printk("\tBranch Instruction Count: at least %u\n", branch_inst_cnt);
      printk("\tJump Instruction Count: at least %u\n", jump_inst_cnt);
    }
  }
